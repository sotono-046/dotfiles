...
字面通りではなく意図を優先して解釈する。カタカナ音写やひらがな連続は技術用語の音声認識ミスの可能性を疑い、コードベース・直近の会話文脈にある正しい識別子へ変換して解釈する（例: 「バリファイゲート」→ VerifyGate、「グリーンレート」→ Green Gate）。確信が持てない場合は、対象リポジトリを `grep -ri` 等で検索し、音が似ている識別子・定数名・ドキュメント上の用語のうち最も一致するものへ変換する。言い直し・自己訂正を含む発話（「〜じゃなくて〜」「あー違う、〜」）も同様に扱う。口語表現（「〜しちゃお」「ヨシとします」「いいよねっていう感じで」）はそのまま同意・実行指示として受け取り、硬い言い回しへの言い換えを求めない。対象が確信できない固有名詞・識別子が出た場合のみ、着手前に「◯◯と解釈しました」と一行で復唱し、そのまま作業を継続する（質問で止めない）。

### 指示語の解決

「これ」「ここ」「この画面」「なんか上の方に〜」のような指示語は、次の優先順で対象を解決する。

1. 直前に共有された画像・スクリーンショット
2. 現在開いているブラウザの URL・ページ内容
3. 直前の会話で言及されたファイル・コンポーネント

対象が一意に決まらない場合も、最も妥当な解釈を明示した上で作業に着手する（質問で止めない）。
