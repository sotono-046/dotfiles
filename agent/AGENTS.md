...
### 並列修正は「サブエージェント経由で Codex を起動・モニター」する

レビュー指摘の一括修正など、独立した修正タスクを並列でこなす場合は、**サブエージェントを直接コードを書く実行者にせず、サブエージェント配下で Codex（`mcp__codex__codex` / `mcp__codex__codex-reply` / `codex exec`）を起動して実際の修正を委譲し、その進行をモニター・回収させる**。これは「Opus は意図と本質、Codex は漏れなき実装」という下請け構造（`opus-codex-orchestration` スキル）を並列修正にも適用するもの。

- 司令塔（Opus 等）→ 修正サブエージェント（並列）→ Codex（実装オペレーター）の3層で進める。
- 各修正サブエージェントは「自分でコードを書かず Codex に委譲」「Codex 出力をモニターし、担当外編集・スコープ逸脱・取りこぼしがあれば `codex-reply` で差し戻し」「確認後に自分でコミット」を担う。
- 競合回避（同一ファイルの同時編集禁止）は `task-orchestration` の原則どおり、グループ間でファイル集合が重ならないよう束ねてから並列起動する。
- 代表的な適用先: `Review-Fix-Team`（レビュー → 修正オーケストレーション）。

#### Codex を使うときの時間・承認ガード（往復の肥大化と暴走を防ぐ）

過去に「Codex とのプラン往復が長引いて時間を浪費した」「承認前に Codex が実装へ着手しファイルへ書き込んでいた」事例があった。これを抑えるため、Codex を回す際は次を守る。

- **プラン段階の Codex は提案のみ・書き込み禁止。** プラン立案・レビュー往復では Codex に実ファイルを変更させない（read-only 前提で起動し、出力はプラン文面として受け取る）。実装着手はユーザー承認後に限る。承認前にファイル変更跡を見つけたら、適用前に必ず内容をレビューし、想定外なら報告して止める。
- **対話往復は最小限に。** Codex との往復（`codex-reply` 連投）でプランを延々と磨かない。狙いと制約を最初の一発で十分に渡し、往復は「逸脱の差し戻し」「取りこぼしの指摘」に限定する。数往復で収束しなければ打ち切ってサブエージェント／司令塔側で判断する（`opus-codex-orchestration` の打ち切り規則に従う）。
- **長時間化はサブエージェント＋バックグラウンドで吸収する。** Codex の実行が長引く見込みなら、Codex はサブエージェント配下で動かし、サブエージェントを `run_in_background: true` で起動して `TaskOutput` で回収する。司令塔（Opus 等）が同期待ちでブロックしない。
