# Browser Automation with playwright-cli (npm package)

`playwright-cli` コマンドによるエージェント向けの対話型ブラウザ操作。メインの SKILL.md は `npx playwright`（テストランナー等）向け。[SKILL.md](../SKILL.md) のスコープ境界も参照。

```yaml
# 元の skill メタデータ（エディタ・ツール向け）
name: playwright-cli
