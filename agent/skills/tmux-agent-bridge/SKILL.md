...
本スキルは「立ち上がっているエージェントに指示を送る運用」だけを担当する。tmux のレイアウト構築（session / window / pane の作成、Claude / Codex / dev server の自動起動）は、プロジェクトの shell helper、手動 tmux command、または別の起動スクリプトで行う。

- レイアウト構築 → 任意の tmux setup：例として dotfiles の `_launch_tmux_ide_or_init` は worktree path ごとに `ide-<project>-<hash>` session を作り、`Claude / Codex / Shell / Dev Server` の 4 分割を用意する。
- 司令塔 → 各ペインへの指示送信 → 本スキル：既存 session の pane id を `scripts/tmux-agents-list.zsh` で拾い、`scripts/tmux-send.zsh` で本文 + 別 Enter の 2 段送信を行う。

注意：Claude Code の Agent Teams など、各 CLI 独自のチーム機能には依存しない。Codex を混ぜたい / 任意タイミングで外部から送り込みたい / 既存 session を後から操作したいときは本スキルを使う。
...
2. **1 番から 2 番へ初期 role packet を送る**
   pane index 0 の司令塔は、作業指示を出す前に pane index 1 の子エージェントへ役割・制約・報告形式を明示する。これは毎 session の初期化として必須。2 番はこの初期化を受けるまで実装を始めない。

   ```markdown
   You are pane 2, the implementation agent in this tmux session.

   Role:

   - Follow task packets from pane 1 only
   - Edit only the assigned scope
   - Do not commit, push, or run destructive git commands unless pane 1 explicitly tells you
   - Stop and report if scope, ownership, or expected output is unclear
   ...
   ```zsh
   ./scripts/tmux-send.zsh <pane-2-id> --paste --stdin <<'EOF'
   <initial role packet>
   EOF
   ```
...
## デフォルトの役割分担（pane 番号ベース）

ユーザーから明示的な役割指定がない場合、**pane 番号で役割を固定する**。

- **1 番（pane index 0）= 司令塔 / レビュー役**
  意図整理、タスク分割、初期 role packet 送信、task packet 作成、差分レビュー、検証、Git 管理を担当。コード編集は原則しない。
- **2 番（pane index 1）= 実装役**
  司令塔から初期 role packet を受け取った後、task packet に従ってコード編集 / テスト追加 / 機械的修正のみを行う。スコープ拡大・設計判断・Git 破壊的操作はしない。
...
- pane title が `Claude` / `Codex` / `Shell` / `Dev Server` などに設定されている場合は補助情報として使う。ただし役割判断の基準は pane index と、司令塔が最初に送る役割宣言に置く。
...
| worktree 別 session を作る例 | `tmux new-session -d -s "ide-$(basename "$PWD")" -c "$PWD" 'claude'`                                                     |
