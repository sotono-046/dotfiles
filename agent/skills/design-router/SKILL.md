...
name: design-router
description: >-
  デザイン・UI 関連の依頼を受けたときに、どの専門スキル
  （design-principles / muller-brockmann-grid-systems / 8pt-grid-spacing /
  artifact-design / vercel:shadcn など）を読むべきかを決める統括スキル。
  ユーザーが「デザインして」「UI を作って」「レイアウトを整えて」「ダッシュボード
  作って」「マガジン風」「サイドバーの余白」「カードのスペーシング」「Linear 風」
  「Swiss デザイン」「グリッドに揃えて」と言った時点でこのスキルを最初に読み、
  決定木で正しい専門スキルを選んでそちらに委ねる。デザイン系専門スキルそのものは
  ここには書かない（重複・分岐の真実の単一化のため）。
---

# Design Router — どの専門スキルを使うか決めるための索引

デザイン系スキルが複数あり、依頼内容と特性のマッチングを誤ると的外れな出力になる。
このスキルは **判断だけ** を担う軽量レイヤー。本体ロジックは各専門スキルに任せる。

> **使い方**: ユーザーの依頼を読んだら、まず § 決定木 を上から順に当てはめる。
> マッチした専門スキルを `Skill` tool で起動するか、自分で読み込んでから作業に入る。
> 1 タスクで複数スキルを併用するパターンは § 併用 を参照。

## 専門スキル一覧（守備範囲）

| スキル | 守備範囲 | 主な成果物 |
|--------|---------|----------|
| **design-principles** | プロダクト UI（SaaS、ダッシュボード、管理画面、Web アプリ）の **デザイン方向決定**：personality（Precision/Warmth/Sophistication/Boldness/Utility/Data）、カラーパレット、タイポ、影、コンポーネント美意識。Linear / Notion / Stripe / Vercel / Mercury 系の精密ミニマル UI。 | 方向性のコミット → トーン・色・コンポーネント仕様 |
| **muller-brockmann-grid-systems** | エディトリアル／マガジン／レポート／ランディングページの **モジュラーグリッド構築**。Swiss design、International Typographic Style。CSS 変数を真実とした 12 列＋8px ベースライン、subgrid バンド、display type の光学アラインメント、`grid_tokens.py` で雛形生成、`verify_grid.js` (Puppeteer) で 0px 検証。 | グリッド付き HTML/CSS、検証スクリプト |
| **8pt-grid-spacing** | リスト／ナビ／フォーム／カード／モーダル／テーブル等の **余白・gap・行高** を 8 ポイントグリッドで統一。トークン化された 4/8 倍数、近接（項目間 < グループ間）、クリック領域 40+、垂直 edge と水平 inset の分離。 | スペーシング仕様（トークン + 値） |
| **artifact-design**（システム配信） | claude.ai Artifact として配信する **単発の HTML ページ**。報告書、可視化、コミュニケーション用の見栄えあるレンダリング。デザインプロセス（パレット brainstorm → 確定 → 構築）と render-verified の仕組み。 | self-contained HTML（外部依存なし） |
...
- ❌ **No**（色値だけ変える、コピー修正、既存トークンへの単純置換、リネーム、文章校正・議事録清書など視覚デザイン判断を伴わない作業）→ **ルーター終了**。専門スキルは読まずに通常フローで作業する。
...
- **その他のフレームワーク・素の HTML/CSS** → 該当する専門スキルなし。`design-principles` + `8pt-grid-spacing` の組合せで進める。
...
新しいデザイン系スキルを追加したら、§ 専門スキル一覧と § 決定木を更新する。重複が出たら統合か役割再定義を行い、ルーターが単一の真実であり続けるようにする。
