...
# The system is gothic / sans-serif led, set in bundled Noto Sans JP.
# Embedded PDF font names appear as "Noto-Sans-JP" (hyphenated); Hiragino Sans
# / Yu Gothic / Helvetica are local-preview or CI fallbacks that also count.
PRIMARY_FONTS = {"Noto", "NotoSans", "Noto Sans", "Hiragino", "YuGothic",
                 "Yu Gothic", "Helvetica", "Arial"}
...
def verify_target(
    ...
    *,
    ...
    fallback_present = any(
        ...
        for kw in ("NotoSans", "Noto Sans", "Georgia", "Palatino", "Hiragino",
                   "YuGothic", "Yu Gothic", "SourceHan", "Source Han", "Noto",
                   "Helvetica", "Arial",
                   "DejaVu", "Liberation")
    )
    ...
    if not any(exp in font_name for exp in PRIMARY_FONTS for font_name in embedded):
        if not fallback_present:
            issues.append(f"no recognizable font embedded in {out.name}")
        elif os.environ.get("KAMI_ALLOW_FALLBACK_ONLY"):
            # CI / headless boxes lack system gothic fonts (Helvetica, Hiragino,
            # Yu Gothic). Treat "primary missing, fallback present" as a warning
            # there so CI can still gate page-count regressions.
            print(f"  WARN: {name}: gothic primary font not embedded; using fallback")
        else:
            issues.append("gothic primary font not embedded; using fallback")
