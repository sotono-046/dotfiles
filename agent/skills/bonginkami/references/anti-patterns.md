...
| # | Pattern | Bad | Fix |
|---|---------|-----|-----|
| 24 | Japanese AI corporate speak | "企業のDXを推進する一気通貫ソリューション" | Say what it does: "紙の業務フローをオンラインに移す" |
| 25 | English AI corporate speak | "Leverage our platform to unlock synergies" | "Use the platform to share data between teams" |
| 26 | Caption restates the flow diagram | "6種類のソース → 6段のフィルタ → 配合設計 → 学習シャード、4ステップ連結" | Cap gives judgment beyond the figure: "ソースが知識の境界を、フィルタが清浄度を、配合が能力の偏りを決める" |
| 27 | AI tone cliches (JA dashes and connectors) | "本質的にはモデルが予測しているだけで——つまり..." / 多用される全角ダッシュ | Drop the meta-commentary frame, state the conclusion directly. Replace dashes with colons or periods. Self-check: `grep -nE '本質的には|つまり|注目すべきは|だけでなく.*も|[——–]'` |
| 28 | Sans font stack missing Japanese fallback | `font-family: Inter` on a th / h3 containing Japanese | Without a Japanese fallback the browser drops to an arbitrary system font that clashes with the gothic main face. Any element that may render Japanese uses `var(--serif)` (the gothic sans alias) |
| 29 | Caption restates the slide title | Title: "ORM vs PRM の比較" / Cap: "ORM と PRM を比較する" | Cap must add information beyond the title: the tradeoff criteria, the applicable scenario, or the next step it leads to. A synonym rewrite wastes the cap's attention slot |
