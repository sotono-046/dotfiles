...
How to write, not how to lay out. Good typography with bad content is just "polished mediocrity". This document covers the writing principles for both English (EN) and Japanese (JA) output. Shared rules come first; language-specific details are called out where they matter.
...
- JA equivalent: avoid「AI時代を見据え、変革を加速」; rewrite in plain words, drop the keynote-deck phrasing
...
Annotate a term on first use. The annotation expires after 8-10 pages or 10 slides. When the term reappears past its half-life, re-mark it with a shorter cue; do not assume the reader still remembers. Fast-read positions (captions, card subtitles, section summaries) are especially strict.

- First use: "LTV (Lifetime Value, total revenue a user contributes before churning)"
- JA first use:「LTV（Lifetime Value、ユーザーが離脱するまでに生む総収益）」
- After the half-life: "LTV (lifetime value)" /「LTV（顧客生涯価値）」
- Do not re-mark inside the half-life window; it reads like explaining something already known
- Applies to slides, long-doc, and other documents over 8 pages. Resume and one-pager are too short to trigger it

### 8. Foreign-term density in Japanese text

In JA body text, keep unannotated foreign technical terms to ≤ 1 per sentence. More than that, split the sentence or add an inline gloss. Prefer action-word glosses (warmup →立ち上げ, rollout →一回生成を走らせる, credit assignment →どの手順に功績を割り当てるか) over concept-word glosses (ウォームアップ, 軌跡生成, 信用割当). Action words describe what happened; concept words just swap the shell.
...
**Structure**:
1. **Headline** (display weight, sans) + one-line subtitle (regular sans body)
...
**Rules**:
- Length target: English 200-350 words; Japanese 400-600 characters
...
6. Signature (sans 500)
7. Enclosures (if any)

**Rules**:
- Minimal - no decorative elements
- Body in sans (single-weight editorial feel; the warmth comes from spacing and color, not the typeface)
...
**Language notes**:
- Japanese sign-offs can use「敬具」, 「以上」, or a context-appropriate formal closing; pair「拝啓」/「敬具」when the register calls for it
...
- Rigid monthly volume: "N deliverables / sessions per month" (JA:「毎月N件 / N回」). Replace with "by milestone" or "as the cadence requires".
- Exhaustive coverage: "full involvement at every release" (JA:「全リリースに常時参加」). Replace with "deep involvement at major milestones".
- Specific session counts: "20 sessions" (JA:「20回の講座」). Replace with "depth of content" anchors and let scheduling adjust.
- "Full-X" totalizers: "all-around / end-to-end / throughout" (JA:「全方位 / 全工程 / 全期間」). Replace with "key X" or "core X".
...
The without-price variant drops the large-figure hero and keeps the value anchors. Rename the chapter from "Plan & investment" (JA:「協業プランと投資」) to "Directions & value" (JA:「協業の方向性と価値」) so the title matches the content.
...
- Summary (~50 English words or ~80 Japanese characters)
...
**Term annotation half-life**: for decks over 10 slides, re-mark any term that reappears across the 10-slide window. See core principle #7.
...
| Avoid | Why it fails |
|---|---|
| "no-brainer" / "rock-solid" / "back to the drawing board" (JA:「鉄板」「やり直し」「詰む」) | Slang verbs; rewrite as a literal claim |
| "must-see" / "one chart explains it all" / "finally understand X" (JA:「必見」「一目でわかる」「完全理解」) | Bait phrasing; state the figure's actual content |
| "blockbuster" / "killer tool" (JA:「神ツール」「バズる」) | Marketing tone in an engineering figure |
| "flywheel" / "closed loop" (JA:「フライホイール」「クローズドループ」, unless audience is fluent) | Use "data loop" / "continuous improvement" / "four entry types" |

### Avoid product-specific judgments that date fast

Pinning a category to a current product name ages the figure within a quarter, because tools evolve faster than diagrams ship. Frame the paradigm and let the reader map products themselves: prefer "previous-generation tool paradigm / new-generation execution paradigm" over "Cursor is the copilot / Claude Code is the autopilot".

### Slogan to neutral rewrites

| Before (slogan) | After (neutral) |
|---|---|
| "Not reviewed, not done" | "Run three passes before delivery" |
| "Any failure means starting over" | "Any failing step returns to revision" |
| "The last gate before delivery" | "Final check before delivery" |
| "Lock in these habits before subscribing" | "Baseline checks before subscribing" |
...
| 2026.04 | 2026年4月 / April 2026 (use the compact form when horizontal space is tight) |
| -> | → |
...
- Add spaces between Japanese text and Latin product names when it improves readability
...
- [ ] Any Japanese filler like「〜を加速」「〜を実現」「〜に寄り添う」「変革」? Rewrite in plain language.
...
- [ ] Japanese punctuation and Japanese / Latin spacing consistent where applicable?
- [ ] Page count within the document's constraint (resume 2, one-pager 1, letter 1)?
- [ ] Any AI writing cliches? JA:「本質的には」「つまり」「注目すべきは」「〜だけでなく〜も」, dash-pile-ups. EN: em dashes, "It's worth noting", "This means that". See anti-patterns #27.
