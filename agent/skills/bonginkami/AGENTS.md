...
- `references/cross_template_diff_allowlist.json` - CSS variables that may legitimately differ between paired templates (font stacks). No non-suffixed siblings remain, so there are currently no pairs to reconcile.
...
- `assets/showcase/` - README and public-site-only screenshots; excluded from `dist/bonginkami.zip`.
...
- `index.html` (Japanese) - public site entrypoint.
...
- `.github/workflows/release.yml` - tag-triggered workflow that builds and attaches `dist/bonginkami.zip` to the release.
- `dist/bonginkami.zip` - tracked release archive.

Kami is a Japanese-only document system. Reference docs are written in English but describe that Japanese-only system; do not duplicate them per language.
...
- Do not bundle large commercial font files into `dist/bonginkami.zip`; package scripts should exclude them while templates keep stable local-preview paths.
- Do not bundle README/public-site-only showcase screenshots into `dist/bonginkami.zip`; keep them under `assets/showcase/` and exclude that directory in `scripts/package-skill.sh`.
...
- When refactoring `scripts/build.py`, `scripts/stabilize.py`, or package helpers into new modules, confirm every new helper file is tracked by Git. `scripts/package-skill.sh` packages from `git ls-files`, so untracked modules pass local imports but disappear from `dist/bonginkami.zip`.
- Any source change that adds scripts, templates, reference JSON, workflows, or package inputs must refresh and inspect `dist/bonginkami.zip`; package freshness is part of release readiness, not a later cleanup step.
- Changes to `SKILL.md`, templates, scripts, references, or package inputs must decide explicitly whether `dist/bonginkami.zip` needs refresh. If the behavior is shipped through the skill package, rebuild and inspect the ZIP before handoff.
...
- Edits to `.github/workflows/*.yml` should be validated on a feature branch (push, watch the run go green) before merging to `main`. Local font / dependency / runner assumptions diverge from CI more often than expected: this project has already burned commits on `pip` cache requiring a manifest, the `fallback_present` set missing Ubuntu defaults (DejaVu / Liberation), and CI never having the macOS system gothic fonts (Helvetica / Hiragino / Yu Gothic).
...
- `dist/bonginkami.zip` is a tracked release archive. Packaging changes must update and inspect it deliberately.
...
8. Diagram template changes must sync to index showcase SVGs: any visual fix to `assets/diagrams/*.html` must also be applied to the matching mini SVG in `index.html`.
...
- Packaging changes: run `bash scripts/package-skill.sh` and confirm `dist/bonginkami.zip` stays small enough for release upload.
- Documentation-only changes: check links and references.

## Release Notes

For public releases, keep notes concise and bilingual when requested. Use one-to-one English and Japanese changelog items, 5 to 8 items, one sentence each.

## Release Flow

- `bash scripts/package-skill.sh` writes the tracked `dist/bonginkami.zip` release archive. It bundles the Noto Sans JP (Regular+Medium) woff2 fonts (OFL) and excludes README/public-site-only showcase screenshots. The size ceiling is 12MB to fit the embedded fonts.
- `dist/bonginkami.zip` should be committed with release changes and uploaded to the latest GitHub release asset when refreshing the Claude Desktop package.
- README and install docs point to the `bonginkami.zip` asset on `bonginkan/design-bonginkami` Releases; prefer refreshing that asset for small packaging or documentation fixes instead of creating a new tag.
- Create a new version tag only when the maintainer explicitly wants a versioned release.

## Fonts

- The system is gothic / sans-serif led, set in a single Japanese typeface: **Noto Sans JP** (Google Fonts, OFL). Bundled as woff2 in two weights — `NotoSansJP-Regular.woff2` (400) + `NotoSansJP-Medium.woff2` (500), embedded in PDF. License text: `assets/fonts/NotoSansJP-OFL.txt`. OFL permits redistribution, so the ZIP bundles it.
- Weight design is two steps only: body = Regular (400), headings / emphasis = Medium (500). Do not use or bundle Black / Bold / Light; headings are set in Medium for restraint, not synthetic bold.
- Font stack: `"Noto Sans JP", "Hiragino Sans", "Yu Gothic", YuGothic, "Helvetica Neue", Arial, sans-serif`. Noto Sans JP (bundled) leads; Hiragino Sans / Yu Gothic are macOS system fonts for local preview; Helvetica Neue / Arial are the final fallback.
- The CSS variable names `--serif` / `--sans` are kept as aliases; both resolve to the Noto Sans JP gothic stack (`--sans: var(--serif)`).
- Run `bash scripts/ensure-fonts.sh` when the bundled woff2 files are missing or truncated; it rebuilds them from the Google Fonts source (`google/fonts` `ofl/notosansjp` variable font), instancing the wght axis at 400 and 500, converting to woff2 (fonttools), and unifying the name-table family to `Noto Sans JP`.
