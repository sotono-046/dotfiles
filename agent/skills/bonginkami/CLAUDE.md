...
- 不打包大体积商业字体到 `dist/bonginkami.zip`，但模板要保留稳定的本机预览路径。
- `dist/bonginkami.zip` 是 tracked release 制品。小修通常刷 latest release 资源即可，不必新 tag。
- 改 build / stabilize / packaging 相关代码后，刷新并检查 `dist/bonginkami.zip`；新增 helper/module/reference JSON 后，确认文件已被 Git 跟踪并进入 package。
