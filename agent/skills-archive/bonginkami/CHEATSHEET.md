...
4. One gothic sans-serif font per page (headings + body). `--serif` and `--sans` are CSS aliases for the same sans-serif family; the variable name `--serif` is kept but resolves to a gothic sans
5. Heading weight locked at 500, no bold
6. Line-height: headlines 1.1-1.3 / dense 1.4-1.45 / reading 1.5-1.55
7. Letter-spacing: body 0 (和欧混植とも、日本語に含まれる英数字も同じ); small labels and all-caps overlines get +0.2-1pt
...
日本語（に含まれる英数字も含めて）を一つのゴシック sans-serif で組む: **Noto Sans JP** (Google Fonts, OFL), bundled in Regular (400) and Medium (500). Noto Sans JP はラテン字も同じ書体でカバーするので、本文中の英単語・数字も同一書体で組める。 The variable name `--serif` is kept but resolves to this sans-serif family; `--sans` always equals `var(--serif)`.

```css
--serif: "Noto Sans JP", "Hiragino Sans",
         "Yu Gothic", YuGothic, "Helvetica Neue",
         Arial, sans-serif;
--sans:  var(--serif);
--mono:  "JetBrains Mono", "Fira Code", "SF Mono",
         Consolas, Monaco,
         "Noto Sans JP", "Hiragino Sans", monospace;
```

Any font-family that may render Japanese must include a Japanese fallback, including `@page` footer text, `pre`, `code`, and SVG labels. A pure mono stack can render missing glyph boxes in WeasyPrint.
...
| Timeline body | `.tl-body` | 9pt / lh 1.40 | 8.5pt (JA) |
...
| Need                | Use                                                            |
| ------------------- | -------------------------------------------------------------- |
| Headline            | sans-serif 500, line-height 1.10-1.30                          |
| Reading body        | sans-serif 400, 9.5-10pt, 1.55 (和欧混植とも)                  |
...
| Chapter start       | sans-serif heading + 2.5pt brand left bar                      |
...
| Metric labels (4-col) | Soft cap 14-18 chars at 9pt Noto Sans JP; trim context, don't wrap (production.md #18). |
...
Not on the table -> first principles: **the gothic sans-serif carries authority, warm gray carries rhythm, ink-blue carries focus**.
