...
Linux first-time:
```bash
apt install -y libpango-1.0-0 libpangoft2-1.0-0 fonts-noto-cjk fonts-noto-sans
```
...
The design system is sans-serif (gothic) led and Japanese-only, set in a single typeface: **Noto Sans JP** (Google Fonts, woff2, OFL), bundled and PDF-embedded so Japanese text — including the Latin letters and digits embedded in it, which Noto Sans JP also covers — renders consistently even on machines without a system gothic. Hiragino Sans / Yu Gothic and Helvetica Neue / Arial act only as system fallbacks. Weight design is two steps: Regular (400) for body, Medium (500) for headings / emphasis — no Black / Bold / Light.

**Most stable setup**: bundled woff2 alongside the HTML / in the project font tree, `@font-face` with relative path.
...
/* --serif / --sans are kept as variable names but both resolve to the sans stack */
body { font-family: var(--sans); }
</style>
```

**Fallback chains** are embedded in every template:

```css
/* One Japanese gothic stack (Latin glyphs covered by the same face) */
font-family: "Noto Sans JP", "Hiragino Sans",
             "Yu Gothic", YuGothic, "Helvetica Neue",
             Arial, sans-serif;
```
...
**Bundled font keeps release ZIPs small**: Noto Sans JP ships as woff2 (Regular + Medium), which are an order of magnitude smaller than full TTFs, so Claude.ai / Desktop skill upload and execution stay within timeout. Release ZIPs are generated with `scripts/package-skill.sh`, which bundles the woff2 files. Templates keep local-first `@font-face` paths plus the system-font fallback chain above.

**Standalone HTML export** (sending a filled HTML file to someone else): Japanese text (including embedded Latin letters and digits) renders acceptably anywhere thanks to the system-font fallback chain, but for an exact match ship the woff2 alongside the HTML. If you cannot, use the PDF output instead.

If you do need to share HTML with the bundled font: the woff2 files and the HTML must live in the same directory, and the `@font-face src` must use a bare filename with no path prefix:
...
Remove the `../fonts/` prefix that templates use when fonts are in the project tree. The recipient must place the `.woff2` files alongside the `.html` file before running WeasyPrint. When in doubt, deliver the PDF.
...
@page {
  @top-right {
    ...
    font-family: var(--sans); font-size: 9pt; color: #6b6a64;
  }
  @bottom-center {
...
| H2 chapter | 32pt | Sans 500 |
| H3 subtitle | 20pt | Sans 500 |
| Body | 18pt | Sans 400 |
| Caption | 14pt | Sans 400 |
| Footer | 12pt | Sans 400 |

Font stack on PowerPoint (single-font-per-page rule):
- JP (Latin glyphs covered by the same face): `Noto Sans JP` -> `Hiragino Sans` -> `Yu Gothic` -> `Arial`

### Nine standard layouts

1. **Cover**: parchment background, centered display title + brand-colored short line + subtitle / author / date
2. **Contents**: parchment, left-aligned `01  Chapter title` (number sans brand-colored)
3. **Chapter divider**: full brand ink-blue background, centered white title - the **only** fully chromatic slide in the deck
4. **Content slide**: eyebrow (sans stone) + core claim (sans near-black) + brand line + body (sans dark-warm)
5. **Data slide**: top takeaway + 2-4 metric cards (big number sans brand + small label sans olive)
...
7. **Pipeline**: eyebrow + title + sans numerals 01/02/03 + step title + step description, laid out in equal-width columns. All steps visible at once (no click-reveal). Use `pipeline_slide()`.
8. **Quote**: parchment, minimal, centered sans quote + `- Source`
...
PARCHMENT = RGBColor(0xf5, 0xf4, 0xed)
BRAND     = RGBColor(0x1B, 0x36, 0x5D)
# SERIF / SANS are kept as variable names (legacy aliases) but both point at the gothic stack.
# JP: Noto Sans JP (Latin glyphs covered by the same face), falling back to Hiragino Sans / Yu Gothic / Arial.
SERIF = "Noto Sans JP"
SANS  = SERIF
...
Print and slide share the same palette and sans typeface, but sizing is different.
...
If the output shows `DejaVuSans` / `Bitstream Vera` - your specified font didn't load, fell through to system ultimate fallback. Expected: the bundled `NotoSansJP` (`Noto-Sans-JP`), or a system gothic fallback such as `HiraginoSans`, `Hiragino-Kaku-Gothic`, `YuGothic`, `Helvetica`/`Helvetica-Neue`, or `Arial`.
...
| `.proj-text line-height` | 1.40 | 1.38 (JA) / 1.40 (EN) |
| `.tl-body font-size` | 9pt | 8.5pt (JA only) |
...
**Root cause**: bundled woff2 missing / truncated, and no system gothic to fall back to.

**Fix**:

```bash
# Preferred: verify (and rebuild if missing) the bundled Noto Sans JP woff2.
# ensure-fonts.sh checks the woff2 are present and non-truncated; if not it fetches
# the Google Fonts source (google/fonts ofl/notosansjp variable font), instances
# the wght axis at 400 and 500, converts to woff2 (fonttools via uv or python3),
# and unifies the name-table family to "Noto Sans JP".
bash scripts/ensure-fonts.sh

# Or put the bundled woff2 alongside the HTML
cp assets/fonts/NotoSansJP-*.woff2 workspace/

# macOS already ships Hiragino Sans / Yu Gothic and Helvetica Neue — no install needed

# Linux system install (system gothic fallback)
apt install fonts-noto-cjk fonts-noto-sans
mkdir -p ~/.fonts && cp *.woff2 ~/.fonts/ && fc-cache -f
```

### 5. (P2) CJK and Latin crowding (Japanese mode only)
...
### 6. (P2) Full-width vs half-width spaces (Japanese mode)

- **Between Japanese characters**: U+3000 full-width space + `·` + space
...
**Root cause**: Equal-flex columns at gap `G` and content width `W` give each column `(W - G·(N-1)) / N` total. After the metric value (typically 12-18mm at 14pt sans), the label has only what remains - usually 22-28mm for 4 columns at 184mm width.

**Fix**: Plan label text against the available budget before layout. Approximate budget at 9pt sans:
