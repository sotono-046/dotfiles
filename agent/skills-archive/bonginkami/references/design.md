...
kami's aesthetic compresses into one sentence: **warm parchment canvas, ink-blue accent, a single gothic face carries hierarchy through weight, avoid cool grays and hard shadows**.

This is not a UI framework. It is a constraint system for print, designed to keep pages stable, clear, and readable.

This system targets Japanese-only documents (the English landing page has been removed; the only supported language is Japanese).
...
4. A single gothic (sans-serif) face carries everything — headlines and body — in Japanese (including the Latin letters and digits embedded in Japanese text, which Noto Sans JP also covers). No second face; reserve weight, not family, for hierarchy
5. Gothic heading weight locked at 500, no synthetic bold
6. Line-heights: tight headlines 1.1-1.3, dense body 1.4-1.45, reading body 1.5-1.55
7. Letter-spacing: body 0 (和欧混植とも、日本語に含まれる英数字も同じ); tracking only for short labels and overlines
...
This system is a fusion of Anthropic's visual language and real Japanese resume iteration. Details below.
...
--olive:       #504e49;   /* Subtext - descriptions, captions. Gothic faces carry enough weight that no per-language darkening is needed. */
...
```css
/* Single Japanese gothic face: Noto Sans JP (which also covers the
   Latin letters and digits embedded in Japanese text).
   The CSS variable names --serif / --sans are kept as aliases; both
   resolve to the gothic stack (--sans always equals var(--serif)). */
...
/* Mono, with Japanese fallback for comments and labels */
...
Bundled font: **Noto Sans JP** (Google Fonts, OFL) woff2, Regular (400) + Medium (500), embedded for PDF. Hiragino Sans / Yu Gothic are macOS system fonts used as local-preview fallbacks. Any font-family that may render Japanese must include a Japanese fallback, including `@page` footer text, `pre`, `code`, and SVG labels. A pure mono stack can render missing glyph boxes in WeasyPrint.
...
- **Body**: 400 (Noto Sans JP Regular)
- **Headings / labels / emphasis**: 500 (Noto Sans JP Medium, a real weight, not synthetic)
- **Forbidden**: 700+ bold, 900 black, 300 light, 100 thin — only 400/500 ship

**Design principle**: the gothic face uses only two weights (400/500), no synthetic bold (600/700), maintaining restrained typography. Hierarchy comes from size and color, with Medium as the single emphasis step.

- `strong { font-weight: 500 }` in long-doc templates locks bold to the Medium file, preventing browsers from synthesizing 700 on top of it
- Noto Sans JP ships Regular (400) and Medium (500) as separate woff2 files, so both weights embed in PDF; WeasyPrint never has to synthesize bold
...
| JA screen body | 1.55-1.65 | Japanese gothic body at slide scale (27-33px) needs looser line-height than the print x1.33 value |
...
- Body text: **0** (和欧混植とも、日本語に含まれる英数字も同じ) — the gothic face needs no density compensation
- Japanese display text (24pt+): **0 to 0.5pt** optional optical spacing for breathing room at large sizes; keep it subtle and scale with font size
...
Proposal A / B / C structure: each module gets a brand-colored letter, a Japanese title, and an uppercase English subtitle.
...
Digits: sans 500, 44pt, `tabular-nums`, `letter-spacing: 0.5pt`. Without the letter-spacing, large digits crowd each other and read as too dense.
...
| Need | Use |
|---|---|
| Big headline | sans 500, size by level, line-height 1.10-1.30 |
| Reading body | sans 400, 9.5-10pt, line-height 1.55 (和欧混植とも) |
...
| Start a chapter | sans 500 heading + 2.5pt brand left bar |
| Cover page | Display-size heading + right-aligned author/date + heavy whitespace |
| Data card | ivory background + 8pt radius + sans 500 big number + sans small label |

Not on this table -> return to first principles: **the gothic face carries hierarchy through weight, warm gray carries rhythm, ink-blue carries focus**.
...
- Caption `.line`: italic sans, 14px olive. Poetic one-liners describing each screenshot
...
- Amount: 112px sans, letter-spacing 0
...
- Flex row with 32px gap, each metric is value (36px sans 500) + label (13px `--latin-ui` stone)
...
- Ethos: closing italic sans line, olive color, max-width 360px. The italic voice signals a personal sign-off
