...
each to Japanese before publishing.
...
Usage:
    ...
    python3 scripts/draft-release-notes.py \\
        ...
        --subtitle-ja "プラグイン導入の修正と監査クリーンアップ。"
...
def render(
    ...
    out.append("### 変更履歴")
    out.append("")
    out.append(f"<!-- サブタイトル: {subtitle_ja} -->")
    out.append("<!-- 上の英語項目に対応させて翻訳し、一対一を保つ -->")
    out.append("")
    for i in range(1, len(commits) + 1):
        out.append(f"{i}. **TODO**：（英語の第 {i} 項に対応）")
    out.append("")
    out.append(_FOOTER)
    return "\n".join(out)
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--subtitle-ja",
        default="<一行の日本語サブタイトル>",
        help="Short Japanese subtitle, kept as a comment hint for translators",
    )
