...
def test_dist_package_contents() -> None:
    archive = REPO_ROOT / "dist" / "bonginkami.zip"
    check("dist/bonginkami.zip exists", archive.exists(), f"missing {archive}")
    if not archive.exists():
        return

    size_bytes = archive.stat().st_size
    check("dist/bonginkami.zip stays below 12MB",
    ...
    check("dist/bonginkami.zip excludes showcase screenshots",
          not forbidden,
          f"forbidden entries: {', '.join(forbidden)}")
    check("dist/bonginkami.zip keeps logo.svg",
          "assets/images/logo.svg" in names)
    check("dist/bonginkami.zip bundles Noto Sans JP fonts",
          "assets/fonts/NotoSansJP-Regular.woff2" in names
          and "assets/fonts/NotoSansJP-Medium.woff2" in names)


# --------------------------- shared registry ---------------------------

def test_registry_consistency() -> None:
    check("HTML_TEMPLATES has 8 entries", len(HTML_TEMPLATES) == 8,
          f"got {len(HTML_TEMPLATES)}")
    check("SCREEN_TARGETS has 1 entry", len(SCREEN_TARGETS) == 1,
    ...
    check("PPTX_TARGETS has 1 entry", len(PPTX_TARGETS) == 1,
          f"got {len(PPTX_TARGETS)}")
    check("PARCHMENT_RGB is canonical", PARCHMENT_RGB == (0xF5, 0xF4, 0xED))


def test_html_templates_keep_single_font_stack() -> None:
    """Every template must keep --sans pinned to --serif (single gothic font per page)."""
    offenders: list[str] = []
    for spec in HTML_TEMPLATES.values():
        source = spec.source
    ...
    check("HTML templates keep --sans: var(--serif)",
          not offenders,
...
def test_pair_names_has_no_pairs_in_single_language_repo() -> None:
    # The repo is English/Japanese only; templates no longer ship a
    # non-suffixed sibling, so there are no CN/EN pairs to reconcile.
    check("pair_names is empty (no non-suffixed siblings)",
          _pair_names() == [],
          f"got {_pair_names()!r}")
...
def main() -> int:
    ...
    test_html_templates_keep_single_font_stack()
    ...
    test_pair_names_has_no_pairs_in_single_language_repo()
