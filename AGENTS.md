...
1. ユーザーに提案
1. 承認後、プランをファイルとして保存
1. Notion MCP でドキュメント保存

### レビューの方法

- レビューは自分のセッション内で行わず、別の単発セッションを作成して行う。
- 例、Claude にレビューであれば`claude -p レビューの内容`というコマンドを使用する。
- 例、codex にレビューであれば、

### マークダウンファイル作成

- 指示されない限り基本的に`[プロジェクト名]/.temp`にファイルを作成
- ファイル名形式: `YYMMDD-<タイトル>.md`
- 日付は`date +%y%m%d`コマンドで明示的に取得
...
- 各プロジェクトで定められた Notion の DB ID を使用する
- プラン承認後に保存

### SOW の作成

- フェーズごとに分けて 1 つのファイルにまとめる
- フェーズごとにコミットと Codex MCP レビューを行う
...
- 長文をワンライナーで送ると失敗するため、temp フォルダに下書き md を作成して使用
